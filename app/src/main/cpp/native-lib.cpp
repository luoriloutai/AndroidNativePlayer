...
static void *renderRoutine(void *pVoid) {
    EnvArgs * args=(EnvArgs*)pVoid;
    JavaVM * vm=args->vm;
    JNIEnv *env;
    vm->AttachCurrentThread(&env, nullptr);
    jobject surface = args->surface;
    jstring url = (jstring)args->url;
...
Java_com_bug_nativeplayer_NativePlayer_openGlPlayVideo(JNIEnv *env, jclass clazz, jstring url, jobject surface) {
    ...
    EnvArgs * args = new EnvArgs();
    env->GetJavaVM(&args->vm);
    //args->url=env->GetStringUTFChars(url, nullptr);
    args->surface = env->NewGlobalRef(surface);
    args->url = env->NewGlobalRef(url);
    pthread_create(&threadStart, nullptr, renderRoutine, args);
